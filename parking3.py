...
class ScanSubscriber():
    ...
    def scanCallBack(self,msg):
        ...
        if self.yaw is not None and self.yaw!=0:
            ...
            y_zero=(-self.yaw)*self.xy_masked[:,0].max()

            if self.vehicle_x<0.3:
                ...
                y_mask_L=self.xy_masked[:,1] < Y_Left_wall-0.15
                ...
                for i in range(60):
                    #x1_wall_r=np.cos(self.angles[i])*self.scan_ranges[i]
                    #y1_wall_r=np.sin(self.angles[i])*self.scan_ranges[i]
                    x2_wall_r=self.a[i,0]
                    y2_wall_r=self.a[i,1]
                    x_wall_r2.append(x2_wall_r)
                    y_wall_r2.append(y2_wall_r)
                x_wall_r2=np.array(x_wall_r2)
                y_wall_r2=np.array(y_wall_r2)
                ...
                for i in reversed(range(-60,0)):
                    #x1_wall_l=np.cos(self.angles[i])*self.scan_ranges[i]
                    #y1_wall_l=np.sin(self.angles[i])*self.scan_ranges[i]
                    x2_wall_l=self.a[i,0]
                    y2_wall_l=self.a[i,1]
                    x_wall_l2.append(x2_wall_l)
                    y_wall_l2.append(y2_wall_l)
                x_wall_l2=np.array(x_wall_l2)
                y_wall_l2=np.array(y_wall_l2)

                

                slope_R2, intercept_R2, _,_,_ = linregress([x_wall_r2,y_wall_r2])
                Y_Right_wall2=slope_R2*self.xy_masked[:,0]+intercept_R2 

                slope_L2, intercept_L2,_,_,_= linregress([x_wall_l2,y_wall_l2])
                Y_Left_wall2=slope_L2*self.xy_masked[:,0]+intercept_L2
                """
            ...
            #plt.plot(self.a[:,0],Y_Left_wall2,color='r')
            #plt.plot(self.a[:,0],Y_Right_wall2,color='g')
